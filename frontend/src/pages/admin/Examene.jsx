...
export default function ExameneAdmin() {
  ...
  const [form, setForm] = useState({ title: "", description: "", duration_minutes: "", max_attempts: "", passing_score: "", department_id: "" });
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      addToast("Examen creat. Adaugă întrebări.", "success"); setShowCreateModal(false);
      setForm({ title: "", description: "", duration_minutes: "", max_attempts: "", passing_score: "", department_id: "" });
  ...
  const togglePublish = async (exam) => {
    try { await axios.post(`${API}/exams/${exam.id}/${exam.is_published ? "unpublish" : "publish"}?token=${token}`); addToast(exam.is_published ? "Examen retras în ciornă" : "Examen publicat", "success"); fetchAll(); }
    ...
    <div className="min-h-screen pb-12">
      <div className="bg-white border-b px-6 py-9" style={{ borderColor: "var(--line)" }}>
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <button onClick={() => setShowCreateModal(true)} className="btn btn-primary">+ Examen nou</button>
        </div>
      </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  {departments
                    .filter((d) => user?.role === "admin" || d.id === user?.department_id)
                    .map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}
                </select>
                <p className="text-xs text-slate-400 mt-1">Cu un departament ales, examenul apare doar studenților din el.</p>
              </div>
            </div>
